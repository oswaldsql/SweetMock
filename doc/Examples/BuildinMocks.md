...
## Buildin mock for http client 

Sets up a test environment for a class with an  dependency.
Configures a mock for the  to simulate HTTP responses and provides testing
...
The test fixture is created for the `ClassWithHttpClientDependency`, which uses
 to perform REST service calls. The method tests the response and verifies
...
var fixture = Fixture.ClassWithDependencies(config => 
    config.httpClient
        .Send(new HttpResponseMessage(){Content = new StringContent("Hello, World!")})
    );
var sut = fixture.CreateClassWithDependencies();
...
var actualHttpCall = Assert.Single(httpClientLogs.SendAsync());
...
## Build in mock for time provider 

Configures a mock for the , enabling control over the system clock for testing purposes.
...
```
 ...
 > it is recommended to use the [Fake time provider](https://learn.microsoft.com/en-us/dotnet/standard/datetime/timeprovider-overview#faketimeprovider-implementation)

## Build in mock for i options 

Configures a mock for the , allowing default or custom option values to be defined.
