...
public class BookRepositoryTests2
{
    ...
    public async Task TheGuideShouldAlwaysBeAvailable()
    {
        ...
        var fixture = Fixture.ShoppingBasket(config =>
        {
            ...
            var basket1 = Mock.IBasket(c => c.Add(), new(config.CallLog));
            config.basketRepo
        ...
        // Assert
        var actualMessage = fixture.Calls.messageBroker.SendMessage(arguments => arguments.userId == userGuid);
        var sendMessage = Assert.Single(actualMessage);
