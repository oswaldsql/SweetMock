...
internal static class LogExtensionsBuilder
{
    internal static CodeBuilder BuildLogExtensionsClass(this CodeBuilder builder, MockContext context)
    {
        ...
        builder
            //.Add("/*")
            .Scope($"internal class {context.Source.Name}_Logs{context.Source.GetTypeGenerics()}(CallLog log, string? instanceName = null){context.Source.ToConstraints()}", classScope =>
            {
                classScope
