...
internal class ConstructorBuilder(MockContext context) {
    ...
    private void CreateLogArgumentsRecord(CodeBuilder builder, IEnumerable<IMethodSymbol> source)
    {
        ...
        builder
            .Add($"public record {context.Name}_Arguments(")
            .Indent(scope => scope
                .Add("global::System.String? InstanceName,")
                .Add("global::System.String MethodSignature" + (arguments.Count != 0 ? "," : ""))
                .Add(args))
            .Add($") : ArgumentBase(_containerName, \"{context.Name}\", MethodSignature, InstanceName);")
            .BR();
    }
    ...
    private void BuildConstructors(CodeBuilder builder, IEnumerable<IMethodSymbol> constructors)
    {
        foreach (var constructor in constructors)
        {
            builder.Scope(this.ConstructorSignature(constructor), ctor => ctor
                ...
                .Add($"this._sweetMockInstanceName = this._sweetMockOptions.InstanceName ?? \"{context.Name}\";")
                .Add($"this._log(new {context.Name}_Arguments(this._sweetMockInstanceName, \"{context.Name}\"{string.Join("", constructor.Parameters.Where(t => t.RefKind == RefKind.None).Select(t => $", {t.Name} : {t.Name}"))}));")
    ...
    private string ConstructorSignature(IMethodSymbol constructor)
    {
        ...
        return $"protected internal MockOf_{context.Name}({parameterList}System.Action<{context.ConfigName}>? config = null, global::SweetMock.MockOptions? options = null) : base({baseArguments})";
    }

    private void BuildEmptyConstructor(CodeBuilder builder) =>
        builder
            .Scope($"internal protected MockOf_{context.Name}(System.Action<{context.ConfigName}>? config = null, global::SweetMock.MockOptions? options = null)", methodScope => methodScope
                .Add("this._sweetMockOptions = options ?? global::SweetMock.MockOptions.Default;")
                .Add("this._sweetMockCallLog = options?.Logger ?? _sweetMockCallLog;")
                .Add($"this._sweetMockInstanceName = options?.InstanceName ?? \"{context.Name}\";")
                .Add($"this._log(new {context.Name}_Arguments(_sweetMockInstanceName, \"{context.Name}\"));")
                .Add($"new {context.ConfigName}(this, config);"));
}
